...
export async function GET(request: NextRequest) {
  ...
  // Check cache
  const cacheKey = `country-states-${country.toLowerCase()}`;
  ...
  try {
    ...
    // Search for states/provinces in the country
    const input = {
      searchStringsArray: ['state', 'province', 'region'],
      locationQuery: country,
      maxCrawledPlacesPerSearch: 30, // Get states
      language: 'en',
      searchMatching: 'all',
      website: 'allPlaces',
    ...
    const states = items.slice(0, 30).map((item: any, index: number) => {
      ...
      return {
        id: `state-${index}`,
        name: item.title || item.name || 'Unknown State',
        type: 'state',
